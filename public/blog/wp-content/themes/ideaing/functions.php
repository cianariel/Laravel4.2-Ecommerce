...
function carbon_post_thumbnails_in_feeds( $content ) {
    global $post;
	if( has_post_thumbnail( $post->ID ) ) {
        $image = get_the_post_thumbnail_url( $post->ID, 'full', false );
    }
	else
	{
	...
	$datepublishstring = get_the_time('Y-m-d H:i:s', $post->ID);
	$timestamp = strtotime($datepublishstring);
	$datepublish = CarbobFormatTime($timestamp);
  $content = '<p id="carbon-attachment-image">' . "<img src='$image' class='thumbnail' />" . '</p>'.'<p id="carbon-post-category">' . get_permalink($post->ID). '</p>'.'<p id="carbon-post-date">' . $datepublish . '</p>' . $content;
    return $content.get_avatar( $post->post_author, 46 );
}
add_filter( 'the_excerpt_rss', 'carbon_post_thumbnails_in_feeds' );
add_filter( 'the_content_feed', 'carbon_post_thumbnails_in_feeds' );
